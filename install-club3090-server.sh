...
SCRIPT_VERSION="2026-05-15.v0.6.60"
...
def load_chat_conversations_from_storage(index_state):
    ...
    for item in list(index_state.get("conversations") or []) + list(index_state.get("archivedConversations") or []):
        ...
        if not conversation_id:
            continue
        index_by_id[conversation_id] = dict(item)
        if "messages" in item or "attachments" in item or "systemPrompt" in item:
            legacy_embedded.append(sanitize_chat_conversation(item))
    if legacy_embedded:
...
def parse_runtime_log_metric_peaks(text):
    ...
    for raw_line in str(text or "").splitlines():
        ...
        if gpu_kv_match:
            value = round(safe_float(gpu_kv_match.group(1)), 2)
        ...
        if cpu_kv_match:
            value = round(safe_float(cpu_kv_match.group(1)), 2)
        ...
        if prefix_match:
            value = round(safe_float(prefix_match.group(1)), 2)
...
def stream_admin_chat_request(handler, data):
    ...
    try:
        ...
        latency_preview = round(time.time() - start, 3)
