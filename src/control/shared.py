...
def variant_engine_family(spec):
    ...
    if "/vllm/" in compose_rel:
        return "vllm"
    if "/llama-cpp/" in compose_rel or "/llamacpp/" in compose_rel or "/ik-llama/" in compose_rel:
        return "llamacpp"
    selector = str(row.get("selector") or row.get("upstream_tag") or row.get("registry_key") or "").strip().lower()
    if selector.startswith("vllm/"):
        return "vllm"
    if selector.startswith("llamacpp/") or selector.startswith("llama-cpp/") or selector.startswith("ik-llama/"):
        return "llamacpp"
    return ""
...
def _warmup_model_candidates(spec, ready_url):
    ...
    candidates = []
    for value in (
        row.get("served_model_name"),
        row.get("model_id"),
        row.get("profile_model_id"),
    ):
...
def maybe_warmup_variant_runtime(spec, ready_url, *, timeout=240):
    ...
    for model_name in model_candidates:
        ...
        try:
            with urllib.request.urlopen(req, timeout=max(30, int(timeout or 240))) as response:
...
def read_storage_browser_file(root_path, relative_path):
    ...
    payload = _read_debug_transfer_bytes(target)
...
def _parse_hf_download_segment(tokens):
    if len(tokens) < 4 or tokens[:2] != ["hf", "download"]:
        return None
    repo_id = str(tokens[2] or "").strip()
    if not repo_id:
        return None
    local_dir = ""
    filenames = []
    ...
    while idx < len(tokens):
        ...
        if token == "--local-dir" and idx + 1 < len(tokens):
            local_dir = str(tokens[idx + 1] or "").strip()
            idx += 2
            continue
        if token.startswith("--local-dir="):
            ...
            continue
        if token.startswith("-"):
            idx += 2 if idx + 1 < len(tokens) and not str(tokens[idx + 1] or "").startswith("-") else 1
            continue
...
def _parse_simple_hf_download_plan(install_command):
    ...
    for segment in [part.strip() for part in text.split("&&") if str(part or "").strip()]:
        ...
        for alternative in alternatives:
            if not alternative.startswith("hf download "):
                return []
            try:
                parsed = _parse_hf_download_segment(shlex.split(alternative))
            except Exception:
                return []
            if not parsed:
                return []
...
def variant_resource_plan_from_row(row, include_missing=False):
    ...
    plan = _parse_simple_hf_download_plan(variant.get("install_command") or "")
    for target in _hf_download_target_paths(plan):
    ...
    for key, role in (
        ...
        if subpath:
            add_path(os.path.join(model_root, subpath.replace("/", os.sep)), role, subpath)
...
def delete_preset_resources(selector="", variant_id=""):
    ...
    running_stop_rc, running_stop_out = stop_runtime_scope("GLOBAL", plan.get("selector"))
    if int(running_stop_rc or 0) != 0:
        errors.append(
            {
                "path": plan.get("selector"),
                "error": f"failed to stop matching runtimes before delete: {running_stop_out}",
            }
        )
    active_jobs = [
        job
...
def _preset_cache_instance_candidates(selector=""):
    ...
    for instance in rows if isinstance(rows, list) else []:
        ...
        try:
            ...
            cache_path = os.path.join(paths["dir"], "cache")
        except Exception:
            instance_id = str(instance.get("id") or "").strip()
            if not instance_id:
                continue
            cache_path = os.path.join(INSTANCES_DIR, instance_id, "cache")
...
def preset_cache_size_summary_for_row(row, max_age=30.0):
    ...
    for candidate in candidates:
        ...
        if not os.path.lexists(path):
            continue
        count += 1
        total += int(_fast_disk_usage_bytes(path, timeout=2) or 0)
    value = {"cache_size_bytes": int(total or 0), "cache_count": int(count or 0)}
...
def delete_preset_caches(selector="", variant_id=""):
    ...
    if int(running_stop_rc or 0) != 0:
        errors.append(
            {
                "path": plan.get("selector"),
                "error": f"failed to stop matching runtimes before clearing caches: {running_stop_out}",
            }
        )
...
def delete_model_resource_paths(paths):
    ...
    for variant in inventory.get("variants") or []:
        ...
        if any(path in resource_paths for path in requested_paths):
            affected_variants.append(dict(variant))
    errors = []
    removed = []
    stopped_selectors = set()
    for variant in affected_variants:
        selector = str(variant.get("selector") or variant.get("upstream_tag") or "").strip()
        if selector and selector not in stopped_selectors:
            rc, out = stop_runtime_scope("GLOBAL", selector)
            if int(rc or 0) != 0:
                errors.append({"path": selector, "error": f"failed to stop matching runtimes before delete: {out}"})
            stopped_selectors.add(selector)
    for job in (model_install_jobs_snapshot() or []):
...
def _monitor_hf_download_progress(process, prefix, plan, env_map):
    if not plan:
        return
    step_totals = []
    try:
        for step in plan:
            ...
            step_totals.append(sum(int(value or 0) for value in sizes.values()) if sizes else 0)
    except Exception:
        step_totals = [0 for _ in plan]
    ...
    while True:
        ...
        while percent >= next_percent and next_percent <= 100:
            append_audit_text_line(
                f"{prefix} progress {next_percent}% ({_format_progress_bytes(loaded_bytes)} / {_format_progress_bytes(total_bytes)}) elapsed {max(1, int(time.time() - started_at))}s"
            )
            next_percent += 5
        if process.poll() is not None:
            break
...
def _run_hf_download_step(job_id, prefix, step, env_map):
    ...
    for repo_id in repo_ids:
        ...
        append_audit_text_line(
            f"{prefix} built-in downloader fetching {repo_id} -> {local_dir}"
        )
        process = subprocess.Popen(
            argv,
            cwd=CLUB3090_DIR,
        ...
        monitor = threading.Thread(
            ...
            args=(process, prefix, [dict(row, repo_ids=[repo_id])], env),
...
def _run_model_install_job(job_id, model_id, variant_id, install_command):
    ...
    try:
        ...
        if shell_command:
            ...
            if plan and not used_builtin_downloads:
                monitor = threading.Thread(
                    target=_monitor_hf_download_progress,
                    args=(process, prefix, plan, env_map),
    ...
    if rc == 0:
        ...
        if removed_paths:
            append_audit_text_line(
                f"{prefix} removed {len(removed_paths)} partial download target{'s' if len(removed_paths) != 1 else ''} after failure"
            )
...
def start_model_install_job(model_id, variant_id, install_command):
    ...
    if plan:
        env_map = _repo_subprocess_env()
        space_plan = _model_install_space_plan(plan, env_map)
        if not space_plan.get("ok"):
            raise RuntimeError(_format_model_install_space_error(space_plan))
...
def _build_admin_task_command(task_name, instance_id="", variant=""):
    task = _selector_token(task_name)
    variant_name = _selector_token(variant)
    if task not in {"benchmark", "report", "rebench"}:
        raise ValueError("Unsupported admin task")
    runtime = _resolve_admin_task_context(instance_id)
    if task == "benchmark":
    ...
    if variant_name not in {"runtime", "full"}:
        raise ValueError("Unsupported rebench variant")
    if runtime["port"] <= 0 or not port_open(runtime["port"], timeout=0.25):
        raise RuntimeError("Rebench requires a running backend. Start that scope first.")
    parts = [f"URL={shlex.quote(runtime['url'])}", "PREFLIGHT_NO_AUTODETECT=1"]
    if runtime["container"]:
        parts.append(f"CONTAINER={shlex.quote(runtime['container'])}")
    if runtime["served_model_name"]:
    ...
    if runtime["engine"]:
        parts.append(f"ENGINE_KIND={shlex.quote(runtime['engine'])}")
    parts.append(
        "bash scripts/rebench-runtime.sh" if variant_name == "runtime" else "bash scripts/rebench-full.sh"
    )
    return {
        "task": f"rebench-{variant_name}",
        "label": "Rebench Runtime" if variant_name == "runtime" else "Rebench Full",
        "command": " ".join(parts),
        "instance_id": runtime["instance_id"],
        "mode": runtime["mode"],
        "container": runtime["container"],
...
def start_admin_task_job(task_name, instance_id="", variant=""):
    task_info = _build_admin_task_command(task_name, instance_id=instance_id, variant=variant)
    with admin_task_job_lock:
        if admin_task_job.get("active"):
...
def _sanitize_metric_peak_number(value, digits=2):
    try:
        number = float(value)
    except Exception:
        return None
...
def _sanitize_system_metric_peak_gpu_row(row):
    ...
    for key in SYSTEM_METRIC_PEAK_GPU_KEYS:
        value = _sanitize_metric_peak_number(data.get(key))
...
def sanitize_system_metric_peaks(payload):
    ...
    for key in SYSTEM_METRIC_PEAK_CHART_KEYS:
        value = _sanitize_metric_peak_number(charts.get(key))
        if value is not None:
