...
SCRIPT_VERSION="2026-05-07.v4.47"
...
def discover_nvidia_fan_indices_v213_compat():
    if not nvidia_settings_available():
        return []
    rc, out = run_nvidia_settings_v213_compat(["-q", "fans"])
    if rc != 0:
        return []
    found = []
    for match in re.finditer(r"\[fan:(\d+)\]", out or "", re.IGNORECASE):
...
def run_nvidia_assignments_v213_compat(assignments):
    results = []
    success = 0
    for assignment in assignments:
        rc, out = run_nvidia_settings_v213_compat(["-a", assignment])
        text = (out or "").strip()
        if rc == 0:
            success += 1
        results.append(f"{assignment}: rc={rc} {text[-500:]}")
...
def run_nvidia_assignment_batch_v213_compat(assignments):
    if not assignments:
        return 0, []
    args = []
    for assignment in assignments:
        args += ["-a", assignment]
    rc, out = run_nvidia_settings_v213_compat(args)
    text = (out or "").strip()
    return rc, [f"batch rc={rc} assignments={assignments}: {text[-1500:]}"]


def restore_gpu_fans_auto_v213_compat(indices=None):
    available_indices = gpu_indices()
    if indices is None:
        indices = list(available_indices)
    else:
    ...
    if not indices:
        return ["no GPU targets selected for auto restore"]
    assignments = [f"[gpu:{gpu_idx}]/GPUFanControlState=0" for gpu_idx in indices]
    batch_rc, batch_results = run_nvidia_assignment_batch_v213_compat(assignments)
    if batch_rc == 0:
        return batch_results
    success, retry_results = run_nvidia_assignments_v213_compat(assignments)
    return batch_results + retry_results + [f"auto restore success={success}/{len(assignments)}"]
...
def set_gpu_fans(speed=None, auto=False, indices=None):
    # Scoped fan control path using the proven v2.13 headless-X / nvidia-settings flow.
    results = []
    available_indices = gpu_indices()
    if indices is None:
        indices = list(available_indices)
    ...
    fan_objects = discover_nvidia_fan_indices_v213_compat()
    if auto:
        assignments = [f"[gpu:{gpu_idx}]/GPUFanControlState=0" for gpu_idx in indices]
        batch_rc, batch_results = run_nvidia_assignment_batch_v213_compat(assignments)
        results.extend(batch_results)
        success, retry_results = run_nvidia_assignments_v213_compat(assignments) if batch_rc != 0 else (len(assignments), [])
    ...
    batch_rc, batch_results = run_nvidia_assignment_batch_v213_compat(batch_assignments)
    results.extend(batch_results)

    enable_success, enable_results = run_nvidia_assignments_v213_compat(enable_assignments) if batch_rc != 0 else (len(enable_assignments), [])
    results.extend(enable_results)

    direct_success, direct_results = run_nvidia_assignments_v213_compat(direct_assignments) if batch_rc != 0 else (len(direct_assignments), [])
    results.extend(direct_results)

    fan_success = 0
    if fan_assignments and batch_rc != 0:
        fan_success, fan_results = run_nvidia_assignments_v213_compat(fan_assignments)
    ...
    if not ok:
        ...
        legacy_batch_rc, legacy_batch_results = run_nvidia_assignment_batch_v213_compat(legacy_batch_assignments)
        results.extend(legacy_batch_results)
        legacy_fan_success = 0
        if legacy_batch_rc != 0:
            legacy_fan_success, legacy_fan_results = run_nvidia_assignments_v213_compat(legacy_fan_assignments)
    ...
    if not ok:
        failover_results = restore_gpu_fans_auto_v213_compat(indices)
