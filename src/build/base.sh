...
SCRIPT_PHYSICAL_PATH="$(readlink -f "${BASH_SOURCE[0]}" 2>/dev/null || printf '%s\n' "${BASH_SOURCE[0]}")"
SCRIPT_PHYSICAL_DIR="$(dirname "${SCRIPT_PHYSICAL_PATH}")"
SCRIPT_CHECKED_OUT_BRANCH="$(git -C "${SCRIPT_PHYSICAL_DIR}" branch --show-current 2>/dev/null || true)"
SCRIPT_SELF_UPDATE_BRANCH_DEFAULT="${SCRIPT_CHECKED_OUT_BRANCH:-__CLUB3090_SELF_UPDATE_BRANCH__}"
CLUB3090_SELF_UPDATE_BRANCH="${CLUB3090_SELF_UPDATE_BRANCH:-${SCRIPT_SELF_UPDATE_BRANCH_DEFAULT}}"
CLUB3090_SELF_UPDATE_REF="${CLUB3090_SELF_UPDATE_REF:-refs/heads/${CLUB3090_SELF_UPDATE_BRANCH}}"
